...
.venv/bin/python -m pytest tests/          # 332 个测试（含离线全链路 e2e 与审计回归）
...
# L3 反事实重放终审（binary_search 候选 → 候选步消息干预重放 k=3 窗口，
# validated +0.2 / refuted −0.3，supersede 进合并归因视图）：
.venv/bin/atap run --config configs/pipeline_cf_replay.yaml --out runs/cf_replay
...
> ⚠️ **离线 L3/DoVer 的 verdict 同样是构造性回环**（与上文"恢复 6/6"
> 同级的 caveat，适用于"六故障 GT 候选全 validated、症状步候选 refuted"
> 及 dover Validated 6/6 等一切离线 verdict 数字）：沙盒中间件只在编辑
> 落在故障 **onset 步**且编辑文本命中故障名时才消费编辑并置
> `fault_removed`，而伪判官只在候选步命中可检测症状签名时才给出含故障
> 名的修复文案——因此 `validated ⟺ 候选步 == GT onset 步`，是归因命中
> 的又一个确定性函数，不构成"滤伪因果"机制的独立证据；且会误伤真阳性
> （rg_ug 在 retrieval_detour 上给出 step=3 == GT 步的候选仍被判
> refuted，因为该步 oracle 是泛化文案）。机制需真实模型 oracle 复跑后
> verdict 才不再是 GT 的函数。

评审修复（第二轮，2026-08-27，**332 测试全绿**，demo/v3/dover 验收数字
回归不变）：

* **导出泄漏 P0**：rerun 轨迹 meta 的 `origin_fault`（GT 完整副本，链式
  再归因用）与 `fault_removed`（构造侧信号）此前绕过导出防泄漏黑名单
  （黑名单只剥 `injected_fault`）——重跑轨迹一经落盘再 `atap export`，
  kind/agent/mast_code/step 整份 GT 从两个导出器外泄。已加入 deny-list
  并加双导出器字符串级回归；
* **cf_replay 判决无下游效果 P1**：L3 调整后的 Hypothesis 与上游原件在
  `bundle.hypotheses()` 合并视图中并存，refuted（−0.3）在下游一切
  `max(confidence)` 选择（t*、对比评测）中永远输给未审原件，"滤伪因果"
  在默认装配下不生效；且该算法此前没有任何 config 装配（唯一集成路径不
  存在，故长期未被发现）。引入 **supersede 语义**（终审 artifact 声明
  `supersedes`，读取时同 (agent,step) 的上游原件被调整副本替换；上游
  artifact 本身不动，可审计），补 `configs/pipeline_cf_replay.yaml`
  装配。端到端：18 条故障 15 validated / 3 refuted——3 例恰为
  binary_search 在 step_repetition 上的已知错步（定位 8 vs GT 5），被 L3
  正确标伪；
* **dover 重跑口径 P2**：`n_repeats` 次确定性重放此前全部计入
  `report.n_reruns`（DoVer 档重跑数在跨算法对比中被放大 n_repeats 倍，
  `reruns=6(ok=6)` 实为 2 次干预 ×3 次确定性重复）——现只有代表性重放
  （最后一次重复）进入 `bundle.reruns`，全部重复仍记录在 attempts
  （`replay_trace_ids` + 成功计数）；
* 契约与小修：`all_at_once`/`targeted_rerun` 成功轨迹从裸 return 改为
  显式 `success_no_attribution`/`skipped_success` artifact（下游可区分
  "跑了但跳过"与"没配"）；drift 共线特征去重从全语料改为按**被比较分组
  对**计算（全局共线 ⇎ 分组内共线，逐对记录 `alias_dedup`）；
  `action_signature._effect` 清除不可达分支；OpenAI 客户端改发
  `max_completion_tokens`（o 系/gpt-5 拒收 `max_tokens`——README 旧记的
  "思考型模型偶发截断"正是这条路径），旧后端 400 时一次性回退
  `max_tokens` 并入重试审计。

...
| all_at_once，关思考 | **33.2%** | 54.3% | $0.16 · 9min |
| binary_search | 11.4% | 34.2% | $0.26 · 32min |

模型选择主导成败——step 命中对思考档不敏感（两档同为 33.2%）、成本低 9 倍；
修正下述评分瑕疵后，思考在手写转写上也只多个位数百分点。沙盒验收数字
（[docs/validation.md](docs/validation.md)）证明管线契约，这一组数字证明它在
真实数据上成立。

> † Magentic-One 的路由后缀曾让手写集 agent 命中被低估，已在 `src/io/whoswhen.py`
> 修复。关思考一行是修复后的原生重跑（手写 agent 8.6% → **43.1%**，总体
> 44.0% → **54.3%**）；开思考一行为后验修正（手写 **46.6%**，总体 **56.0%**）。

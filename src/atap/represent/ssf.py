...
* **失败观测保留**：结构化错误消息（``error:`` / ``exception`` 等前缀，
  :func:`is_error_observation`）→ 保留。原文用词面词典 K，但本框架玩具
  语料本身是"关于错误分析的散文"，词面匹配会把正文误判为错误——故
  默认 ``keyword_mode=strict``（结构前缀）；``loose`` 恢复词面词典行为
  （适用于代码/工具日志域，即原文场景）【推断】；
* 其余长观测 → 替换为**可逆占位符** ``⟦folded:F3 | 摘要…⟧``，原文存入
  side table，可按需展开（investigator 的 inspection tool；阶段二的
  单遍判官只读折叠视图，按需展开留作阶段三 L2 下钻）。

与原文的差异【推断】：占位符附带首行摘要（≤100 字符），让单遍判官仍能
从折叠视图获得最小证据（如检索命中列表），这是对本框架"无下钻工具的
单遍判官"场景的工程适配；原文占位符为纯提示文本。

产物：``{"fold", "table", "stats"}``；不修改原始事件（视图与数据分离）。
原文数字参照：平均 94.6% 的步可折叠、折叠步中仅 20.2% 需按需展开。
"""
